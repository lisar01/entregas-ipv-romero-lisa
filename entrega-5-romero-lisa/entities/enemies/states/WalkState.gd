...
func update(delta):
	...
	var direction_to = parent.global_position.direction_to(next_point)
	parent.animated_sprite.flip_h = direction_to.x < 0
	parent.velocity.x = clamp((parent.velocity.x + direction_to.x * speed), -max_h_speed, max_h_speed)
